<table id="candidateList" align="left"> 
	<thead class="ui-widget-header">
		<tr>
			<td style="width:30px;"><h5>&nbsp;</h5></td>
			<td style="width:282px;"><h5><?php echo __('Name') ?></h5></td>
			<td style="width:282px;"><h5><?php echo __('Driver') ?></h5></td> 
		</tr>
	</thead>
	<tbody  class="ui-widget-content">	
		<?php 
			$i = 1 ;
			 
		foreach($candidates as $candidate ): ?>
		<tr>
			<td style="">
				<input type="radio" id="selectCandidate-<?php echo $i ?>" name="selectCandidate" value="<?php echo  $candidate->assigned_id. '$'.$candidate->plateCode.'$'. $candidate->plateNo.'$'. $candidate->id ?>">
				
			</td> 
			<td class="" style="width:220px;min-width:220px;"> 
				<?php echo $candidate->plateNo .' ( '.$candidate->plateCode.' )' ?>
			</td>
			<td class="" style="width:220px;min-width:220px;"> 
				<?php echo $candidate->id ?>
			</td> 
		</tr>
		<?php $i++; ?>
		<?php endforeach; ?>
	</tbody>
</table>


<script>
 

</script>


