<div class="ui-container-box">
	...
	<?php //$flags = EmployeeTable::processCreate(1, 9, 1, 'asfasdf', $father_name, $grand_father_name, $id_no, $job_title, $birth_date, $status, $vat_number, $project_no, $description, $pobox_no, $mobile_no, $phone_no, $fax_no, $email, $website); 
		...
		//$flag = ProjectTable::processCreate ( 9, 'Norad', $project_code, 1, $project_no, 'NORAD', $description, $street_no, $house_no, $pobox, $mobile_no, $phone_no, $fax_no, $email, $website);
	//$flag
		...
		<div class="ui-list-cont">		
			<div class="ui-main-list-cont">
				<div class="ui-list-header">				
					<h1><img class="clickRefresh" src="<?php echo image_path('setting/staff_medium') ?>"><?php echo __('Project')  ?></h1>		
					...
					<div class="ui-main-content-cont" >	
						<div class="ui-main-content">
							<fieldset class="ui-content-fieldset">
								<legend><img style="width:16px;height:16px;" src="<?php echo image_path('setting/staff_medium') ?>" ><?php echo ('New Project')  ?></legend>
								...
								<table>
									<tr>
										...
										<fieldset  style="">
											...
											<div class="ui-main-form" >		
												 <?php include_partial('projectForm', array()) ?>
											</div><!-- end of ui-content-form -->
										</fieldset>
									</td>
									</tr>
					...
					<div class="ui-main-content-cont" >
						<div class="ui-content-list-box">
							...
							<div class="ui-content-lists">
								<?php include_partial('list', array('projects' => $projects )) ?>
...
<div class="modal" id="parentPrompts">
	<form>
				...
				<div class="ui-main-content-cont" >
					<div class="ui-content-list-box">
						<div class="ui-list-filter-cont">
							...
							<div class="ui-filter-list"> 
								<?php include_partial('candidateParentFilter', array()) ?>
							</div>
							<div class="clearFix"></div>
						</div>
						<div class="ui-content-lists">
							<?php include_partial('candidateParentList', array('parents' => $parents )) ?>
						</div><!-- end of ui-content-lists -->
					</div><!-- end of ui-content-list-box -->
				</div><!-- end of ui-main-content-cont --> 
			</div><!-- end of ui-main-content-list --> 
...
<div class="modal" id="directorPrompts">
	<form>
	<div class="ui-partial-container" style="border:0px solid #f00;">
		 <div class="ui-partial-container">
			<div class="ui-partial-main-list-cont" >
				...
				<div class="ui-main-content-cont" >
					<div class="ui-content-list-box">
						<div class="ui-list-filter-cont">
							...
							<div class="ui-filter-list"> 
								<?php include_partial('candidateDirectorFilter', array()) ?>
							</div>
							<div class="clearFix"></div>
						</div>
						<div class="ui-content-lists">
							<?php include_partial('candidateDirectorList', array('directors' => $directors )) ?>
						</div><!-- end of ui-content-lists -->
					</div><!-- end of ui-content-list-box -->
				</div><!-- end of ui-main-content-cont --> 
			</div><!-- end of ui-main-content-list --> 
...
<script>
	 ...
	 var triggerParent = $(".parentModal").overlay({ 
		mask: {
		color: '#ebecff',
		loadSpeed: 200,
		opacity: 0.9
	...
	$("#parentPrompts form").submit(function(e) {

		if($("input[name=selectParent]:checked", this).length == 0)
			$("input[id=selectParent-1]").attr("checked", "checked");

		triggerParent.eq(0).overlay().close();

		var input = $("input[name=selectParent]:checked", this).val();
		var listArr = input.split("$");

		document.getElementById("parentID").value = listArr[0];
		document.getElementById("parent_name").value = listArr[1]; 
	 ...
	 var triggerDirector = $(".directorModal").overlay({ 
		mask: {
		color: '#ebecff',
		loadSpeed: 200,
		opacity: 0.9
	...
	$("#directorPrompts form").submit(function(e) {

		if($("input[name=selectDirector]:checked", this).length == 0)
			$("input[id=selectDirector-1]").attr("checked", "checked");

		triggerDirector.eq(0).overlay().close();

		var input = $("input[name=selectDirector]:checked", this).val();
		var listArr = input.split("$");
	...
	function createProject()
	{
		...
		var alias = document.getElementById('project_alias').value;
		...
		if( name == '' && parentID == '')
		{
			$('#project_name_validation').removeClass('displayNone');
			$('#project_name').addClass('validation_error_border');
			
			$('#parent_validation').removeClass('displayNone');
			$('#parent_name').addClass('validation_error_border');
		...
		if( name == '')
		{
			$('#project_name_validation').removeClass('displayNone');
			$('#project_name').addClass('validation_error_border');
			$('#parent_validation').addClass('displayNone');
			$('#parent_name').removeClass('validation_error_border');
		...
		if( parentID == '')
		{
			$('#project_name_validation').addClass('displayNone');
			$('#project_name').removeClass('validation_error_border');
			$('#parent_validation').removeClass('displayNone');
			$('#parent_name').addClass('validation_error_border');
			...
			$.ajax({
				type: "GET",
				data: 'name='+name+'&parent_id='+parentID+'&project_no='+project_no+'&status='+status+'&alias='+alias+'&phone_no='+phone_no+'&mobile_no='+mobile_no+'&pobox='+pobox+'&fax_no='+fax_no+'&email='+email+'&website='+website+'&description='+desc,
				url: '<?php echo url_for('project/createProject')?>', 
		...
		//var data = 'name='+name+'&parent_id='+parentID+'&project_no='+project_no+'&status='+status+'&alias='+alias+'&phone_no='+phone_no+'&mobile_no='+mobile_no+'&pobox='+pobox+'&fax_no='+fax_no+'&email='+email+'&website='+website+'&description='+desc;
		//alert(data);
