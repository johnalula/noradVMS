	...
	<tbody>
		...
		<tr class="<?php echo fmod($row, 2) ? 'even' : 'odd' ?>"> 
			...
			<td class="" style="width:15px;min-width:15px;padding:2px 8px;"> 
				<input type="checkbox" id="category-check-<?php echo $i;?>" name="category-check[<?php echo $driver->id;?>]" class="checkcategory"  />
			</td>
			...
			<td  class="" style="padding:0px 0px 0px 0px;width:50px;min-width:50px;border-left:1px solid #ddd;background:#dfe2e7;"> 
				<div class="ui-list-action" style="width:50px;min-width:50px;padding:0x 0px;text-align:center;">
					<ul>
						<li>
							<a href="<?php echo url_for('assignment/view?task_id='.$driver->id.'&token_id='.$driver->token_id) ?>" class="" rel="<?php echo $driver->id ?>">	
						...
						<li> 												
							<button class="ui-button" onClick="Javascript:deleteCategory(<?php echo $driver->id ?>);">
